...
#output into file:
WORKDIR=$ROOTDIR/output/phastcons_workflow/onTheFly
if [ -e $WORKDIR ]; then
mkdir -p $WORKDIR
fi
...
python fimo2bed.py --i $ROOTDIR/data/motif_alignments_onTheFly_dm6.txt --o $WORKDIR/motif_alignments_onTheFly_dm6_BED.bed
#intersect with gene slop
bedtools intersect -a $WORKDIR/motif_alignments_onTheFly_dm6_BED.bed -b $WORKDIR/gene_only_slop.txt -wb > $WORKDIR/bedtoolsintersect_out.txt
