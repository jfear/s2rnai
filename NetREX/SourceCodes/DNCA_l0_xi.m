function [output] = DNCA_l0_xi(Input)
...
%Input.lambda; Input.eta; Input.kappa; Input.mu
%
%Output:
...
% [S, A] = NCA(Input, Input.S0, Input.A0, LapG, 5000);

[Sold, Aold] = NCA(Input, Input.S0, Input.A0, LapG, 5); %NCA_1
...
for k = 1 : Input.IterNum
    ...
    [Snew] = PALM_S_ElasticNet(Input, Sold, Aold, LapG, c(k));
...
function [Anew] = ClosedForm4A_t(Input, Sold)
Anewt = inv(Sold'*Sold+Input.mu*eye(Input.NumTF))*Sold'*Input.GEMatrix;
Anew = Anewt;%(abs(Anewt)<=Input.M).*Anewt + (abs(Anewt)>Input.M)*Input.M.*sign(Anewt);
end

function [S, A] = NCA(Input, Sold, Aold, LapG, IterNum)

Spport = Sold;
Ir = Input.xi*eye(Input.NumGene);
for i = 1 : IterNum
    [Anew] = ClosedForm4A_t(Input, Sold);
    [Snew] = RegressionWithFixSupport(Input, Sold, Anew);%bartelsStewart(2*Input.kappa*LapG+2*Ir, [], [], Anew*Anew', Input.GEMatrix*Anew');%RegressionWithFixSupport(Input, Spport, Anew);
